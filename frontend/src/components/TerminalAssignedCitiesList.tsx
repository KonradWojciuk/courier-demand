...
export default function TerminalAssignedCitiesList() {
  ...
  // Fetch available terminals (combine both sender and receiver terminals)
  useEffect(() => {
    const fetchTerminals = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
        const [receiverResponse, senderResponse] = await Promise.all([
          fetch(`${apiUrl}/api/packets/terminal-monthly-distribution?month=${selectedMonth}&year=${selectedYear}`),
          fetch(`${apiUrl}/api/packets/sender-terminal-monthly-distribution?month=${selectedMonth}&year=${selectedYear}`)
        ]);
        ...
        if (receiverResponse.ok && senderResponse.ok) {
          ...
          const receiverTerminals = receiverResult.data?.map((item: any) => item.terminal) || [];
          const senderTerminals = senderResult.data?.map((item: any) => item.terminal) || [];
          
          // Combine and deduplicate terminals
          const allTerminals = [...new Set([...receiverTerminals, ...senderTerminals])];
          setAvailableTerminals(allTerminals.sort());
          
          if (allTerminals.length > 0) {
            if (!selectedTerminal || !allTerminals.includes(selectedTerminal)) {
  ...
  // Fetch cities assigned to selected terminal
  useEffect(() => {
    if (!selectedTerminal) {
      setAllCityData([]);
      setLoading(false);
    ...
    const fetchCityData = async () => {
      ...
      try {
        ...
        if (viewType === 'monthly') {
          url = `${apiUrl}/api/packets/cities-assigned-to-terminal-monthly?terminal=${encodeURIComponent(selectedTerminal)}&month=${selectedMonth}&year=${selectedYear}`;
        } else {
          url = `${apiUrl}/api/packets/cities-assigned-to-terminal-weekly?terminal=${encodeURIComponent(selectedTerminal)}&startDate=${selectedWeekStart}&endDate=${selectedWeekEnd}`;
        }
    ...
    <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-full border border-gray-800">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-white font-bold text-lg mb-1">Cities Assigned to Terminal</h3>
            <p className="text-gray-400 text-sm">
              Select a terminal to see all cities assigned to it (cities involved in shipments with this terminal)
            </p>
          </div>
        </div>
