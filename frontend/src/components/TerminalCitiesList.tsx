...
export default function TerminalCitiesList() {
  ...
  // Fetch available terminals (always use receiver terminals)
  useEffect(() => {
    const fetchTerminals = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
        const response = await fetch(`${apiUrl}/api/packets/terminal-monthly-distribution?month=${selectedMonth}&year=${selectedYear}`);
        ...
        if (response.ok) {
          ...
          if (terminals.length > 0) {
            // Only set if no terminal is selected or if current selection is not in the list
  ...
  // Fetch cities for selected terminal
  useEffect(() => {
    if (!selectedTerminal) {
      setAllCityData([]);
      setLoading(false);
    ...
    const fetchCityData = async () => {
      ...
      try {
        ...
        if (viewType === 'monthly') {
          url = `${apiUrl}/api/packets/cities-by-terminal-monthly?terminal=${encodeURIComponent(selectedTerminal)}&month=${selectedMonth}&year=${selectedYear}`;
        } else {
          url = `${apiUrl}/api/packets/cities-by-terminal-weekly?terminal=${encodeURIComponent(selectedTerminal)}&startDate=${selectedWeekStart}&endDate=${selectedWeekEnd}`;
        }
    ...
    <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-full border border-gray-800">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-white font-bold text-lg mb-1">Cities by Terminal</h3>
            <p className="text-gray-400 text-sm">
              View cities sending shipments to the selected receiver terminal
            </p>
          </div>
        </div>
