...
export default function SenderTerminalChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (viewType === 'monthly') {
          url = `${apiUrl}/api/packets/sender-terminal-monthly-distribution?month=${selectedMonth}&year=${selectedYear}`;
        } else if (viewType === 'weekly') {
          url = `${apiUrl}/api/packets/sender-terminal-weekly-distribution?startDate=${selectedWeekStart}&endDate=${selectedWeekEnd}`;
        } else {
          url = `${apiUrl}/api/packets/sender-terminal-daily-distribution?date=${selectedDate}`;
        }
        
        const response = await fetch(url);
        
        if (!response.ok) {
          throw new Error(`Failed to fetch sender terminal ${viewType} distribution`);
        }
  ...
  return (
    <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-full border-2 border-purple-600/30">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-white font-bold text-lg mb-1">Number of shipments per terminal</h3>
            <p className="text-gray-400 text-sm">Number of shipments sent from terminals in a given period</p>
          </div>
        </div>
